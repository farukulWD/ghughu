...
export default function Header() {
  ...
  return (
    <header style={{ background: 'var(--paper)', borderBottom: '1px solid var(--line)', position: 'sticky', top: 0, zIndex: 50 }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '18px 24px', display: 'grid', gridTemplateColumns: 'auto 1fr auto', alignItems: 'center', gap: 28 }}>
        {/* Brand */}
        ...
        <label style={{
        ...
        }} className="focus-within:border-[var(--g500)] focus-within:shadow-[0_0_0_4px_rgba(201,162,75,.18)] focus-within:bg-white">
          ...
          />
          <span style={{ fontSize: 11, color: 'var(--ink3)', border: '1px solid var(--line)', borderRadius: 6, padding: '2px 6px', background: '#fff' }}>⌘K</span>
        </label>
      ...
      <nav style={{ borderBottom: '1px solid var(--line)' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 24px', display: 'flex', alignItems: 'center', gap: 28, overflowX: 'auto' }}>
          {[
            ...
            { label: 'Rechargeable Fans · রিচার্জেবল ফ্যান' },
            { label: 'Lamps · শখের লাইট' },
            { label: 'Sarees & Fashion · শাড়ি' },
            { label: "Women's Care · নারীদের প্রয়োজনীয়" },
            { label: 'Home Goods · বাসাবাড়ির জিনিস' },
            { label: 'Beauty' },
            { label: 'Kids & Toys' },
          ].map((item) => (
            <a key={item.label} href="#"
              style={{
                padding: '14px 0', fontSize: 13.5,
